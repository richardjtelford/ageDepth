...
get.value <- function(js, target, numeric = TRUE) {
  x <- js[grep(target, js)]
  x <- gsub(".+=(.+);", "\\1", x)
  if(numeric) {
    as.numeric(x)
...
likelihoods <- get.vector(js, "likelihood\\.prob=")
start <- get.value(js, "likelihood\\.start")
resolution <- get.value(js, "likelihood\\.resolution")
...
depths <- get.value(js, "data\\.z")
posterior <- get.vector(js, "posterior\\.prob=")
comment <- get.value(js, "posterior\\.comment\\[0\\]", FALSE)
comment <- comment[!grepl("MCMC", comment)]

p_start <- get.value(js, "posterior\\.start")
p_resolution <- get.value(js, "posterior\\.resolution")
